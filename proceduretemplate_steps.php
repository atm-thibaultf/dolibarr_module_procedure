...
/**
 *  \file       proceduretemplate_steps.php
 *  \ingroup    procedure
 *  \brief      Tab of steps on ProcedureTemplate
 */
...
if ($object->id > 0) {
	...
	print dol_get_fiche_head($head, 'steps', $langs->trans("ProcedureTemplate"), -1, $object->picto);
...
foreach ($search as $key => $val) {
	if (array_key_exists($key, $step->fields)) {
		...
		$mode_search = (($step->isInt($step->fields[$key]) || $step->isFloat($step->fields[$key])) ? 1 : 0);
		if ((strpos($step->fields[$key]['type'], 'integer:') === 0) || (strpos($step->fields[$key]['type'], 'sellist:') === 0) || !empty($step->fields[$key]['arrayofkeyval'])) {
			if ($search[$key] == '-1' || ($search[$key] === '0' && (empty($step->fields[$key]['arrayofkeyval']) || !array_key_exists('0', $step->fields[$key]['arrayofkeyval'])))) {
				$search[$key] = '';
			}
			$mode_search = 2;
		}
...
if ($search_all) {
	...
	print '<!-- Search done like if PROCEDURETEMPLATESTEP_QUICKSEARCH_ON_FIELDS = '.$setupstring.' -->'."\n";
...
$reshook = $hookmanager->executeHooks('printFieldListFooter', $parameters, $step, $action); // Note that $action and $object may have been modified by hook
print $hookmanager->resPrint;
