...
if ($object->id > 0) {
	...
	if (!empty($user->rights->procedure->proceduretemplatestep->write) || $user->hasRight('procedure', 'allactions', 'create')) {
		print '<a class="butAction" href="'.DOL_URL_ROOT.'/custom/procedure/proceduretemplatestep_card.php?action=create'.$out.'">'.$langs->trans("AddStep").'</a>';
	} else {
		print '<a class="butActionRefused classfortooltip" href="#">'.$langs->trans("AddStep").'</a>';
	}

	print '</div>';

		print load_fiche_titre($langs->trans("StepsOnProcedureTemplate"), '', '');
