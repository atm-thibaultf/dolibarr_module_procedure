...
/**
 *   	\file       proceduretemplate_list.php
 *		\ingroup    procedure
 *		\brief      List page for procedure template
 */
...
require_once __DIR__.'/class/proceduretemplate.class.php';
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('procedure', 'proceduretemplate', 'write');
	$permissiontodelete = $user->hasRight('procedure', 'proceduretemplate', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("procedure")) {
	accessforbidden('Module procedure not enabled');
}
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'ProcedureTemplate';
	$objectlabel = 'ProcedureTemplate';
	$uploaddir = $conf->procedure->dir_output;
...
//$help_url = "EN:Module_ProcedureTemplate|FR:Module_ProcedureTemplate_FR|ES:Módulo_ProcedureTemplate";
$help_url = '';
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/procedure/proceduretemplate_card.php', 1).'?id='.$id);
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/procedure/proceduretemplate_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendProcedureTemplateRef";
$modelmail = "proceduretemplate";
$objecttmp = new ProcedureTemplate($db);
